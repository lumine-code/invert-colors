...
module.exports = {
  ...
  activate() {
    ...
    this.updateBodyClass("invert-colors-transitions", true);
  ...
  deactivate() {
    ...
    for (const className of [
      ...
      this.updateBodyClass(className, false);
    }
  ...
  workspaceUpdate(state) {
    this.updateBodyClass("invert-colors-workspace", state);
  ...
  editorUpdate(state) {
    this.updateBodyClass("invert-colors-editor", state);
  ...
  imageUpdate(state) {
    this.updateBodyClass("invert-colors-image", state);
    if (this.icons.image) {
  ...
  pdfViewUpdate(state) {
    ...
    if (this.icons.pdfView) {
      this.icons.pdfView.classList.toggle("active", state);
    }
  },

  workspaceDocuments() {
    const documents = new Set([globalThis.document]);
    for (const item of lumine.workspace.getPaneItems()) {
      const document = lumine.workspace.getWindowSurface(item)?.document;
      if (document) documents.add(document);
    }
    return documents;
  },

  updateBodyClass(className, state) {
  ...
  observePdfViewer(viewer) {
    ...
    const entry = { viewer };
    this.pdfViewers.set(viewer, entry);
